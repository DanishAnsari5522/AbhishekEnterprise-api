...
const addPurchase = async (req, res) => {
    try {
        let { invoiceNo, date, gstType, supplierName, address, recieverName, gstInvoiceNo, gstInvoiceDate, item } = req.body;
        if (!date || !gstType || !invoiceNo || !supplierName || !address || !recieverName || !gstInvoiceNo || !gstInvoiceDate || !item) {
            return res.status(400).json({ success: false, message: "invoiceNo,date, gstType, supplierName, address, recieverName, gstInvoiceNo, gstInvoiceDate, item   are required" })
        }
        const data = await purchase.create({ invoiceNo, date, gstType, supplierName, address, recieverName, gstInvoiceNo, gstInvoiceDate, item })
